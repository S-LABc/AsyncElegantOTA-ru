...
AsyncElegantOtaClass AsyncElegantOTAru;
    ...
    _server->on("/update", HTTP_POST, [&](AsyncWebServerRequest *request) {
        ...
        if (!index) {
            if(!request->hasParam("MD5", true)) {
                return request->send(400, "text/plain", "Отсутствует параметр MD5");
            }

            if(!Update.setMD5(request->getParam("MD5", true)->value().c_str())) {
                return request->send(400, "text/plain", "Неверный параметр MD5");
            }
                ...
                return request->send(400, "text/plain", "Не удалось запустить обновление");
            }
        }
        ...
        if (final) { // if the final flag is set then this is the last frame of data
            if (!Update.end(true)) { //true to set the size to the current progress
                Update.printError(Serial);
                return request->send(400, "text/plain", "Не удалось завершить обновление");
            }
        }else{
            return;
        }
